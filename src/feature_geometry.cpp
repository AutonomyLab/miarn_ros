...
//                    return false;// is a corner
    ...
    /// was it a line ?
    if (std::fabs(average-M_PI) < .1 && std_dev < 0.2) {
...
bool LaserFeatureX::FindLeg(const int begin,const int end, bool check2legs) {
    ...
    if ( diameter < this->max_leg_diameter && check2legs) {
        unsigned int tmp_leg1,tmp_leg2;
        tmp_leg1=(unsigned int) begin+0.25*(end-begin);
        tmp_leg2=(unsigned int) begin+0.75*(end-begin);
        AddProbableTwoLegs(
...
void LaserFeatureX::LinearRegression(float & m,float &b, float &r, int begin, int end) {
    int nPoints = (int) std::fabs(float(end-begin))+1;
    if (nPoints > 0) {
        float SumX  = 0;
        float SumY  = 0;
