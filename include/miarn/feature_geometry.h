...
segment(int in_begin=0, int in_end=0) : begin(in_begin),end(in_end) {}
    ...
    void AddProbableTwoLegs(float pos_x, float pos_y,float leg1_x,float leg1_y,float leg2_x,float leg2_y,float begin_x, float end_x, float  begin_y, float end_y,int begin_index, int end_index);
    void AddProbableLeg(float pos_x, float pos_y,float begin_x, float end_x, float  begin_y, float end_y,int begin_index, int end_index);
    void AddLine(float slope, float offset,float maxerror, float begin_x, float end_x, float  begin_y, float end_y,int begin_index, int end_index);
    void AddArc(float pos_x, float pos_y, float radius, float begin_x, float end_x, float  begin_y, float end_y,int begin_index, int end_index,float average, float std_dev);
    void AddSegment(float begin_x, float end_x, float  begin_y, float end_y,int begin_index, int end_index);
};
